...
const testUser = { email: 'user@med-logs-delete.welltrack', password: 'password123' };
...
beforeAll(async () => {
  await prisma.user.deleteMany({ where: { email: { endsWith: '@med-logs-delete.welltrack' } } });
...
beforeEach(async () => {
  const log = await prisma.medicationLog.create({
    data: { userId, medicationId: medId, taken: true },
  });
  logId = log.id;
});
...
describe('DELETE /api/medication-logs/:id', () => {
  it('returns 204 and removes the log', async () => {
    ...
    const gone = await prisma.medicationLog.findUnique({ where: { id: logId } });
  ...
  it("returns 403 for another user's log", async () => {
    const otherReg = await request(app)
      .post(REGISTER)
      .send({ email: 'other@med-logs-delete.welltrack', password: 'password123' });
